...
DROP POLICY IF EXISTS "Enable all operations for everyone" ON public.delivery_notes;
...
DROP POLICY IF EXISTS "Enable all operations for everyone" ON public.delivery_note_items;
CREATE POLICY "Enable all operations for everyone" 
...
-- Note: This function should already exist from previous migrations, but we'll create it if it doesn't
...
UNION ALL
SELECT 
    'delivery_note_items' as table_name,
    COUNT(*) as column_count
FROM information_schema.columns 
