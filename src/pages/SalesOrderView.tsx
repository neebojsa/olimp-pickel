...
export default function SalesOrderView() {
  ...
  const printSalesOrder = () => {
    if (!id) return;
    const printUrl = `/sales-orders/${id}/print`;
    window.open(printUrl, '_blank')?.print();
  };

  const downloadPDF = async () => {
    if (!salesOrder || !containerRef.current) {
      toast({ title: "Error", description: "Sales order not found.", variant: "destructive" });
      return;
    }
    setDownloadingPDF(true);
    try {
      const container = containerRef.current;
      const pages = container.querySelectorAll(".so-print-page");
      if (!pages.length) {
      ...
      pdf.save(`SalesOrder_${salesOrder.sales_order_number}_${new Date().toISOString().split("T")[0]}.pdf`);
      ...
      <div className="min-h-screen bg-gray-100 p-4 md:p-6 print:p-0 print:bg-white">
        <div className="flex flex-wrap gap-2 pb-4 print:hidden justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={printSalesOrder} variant="secondary">
