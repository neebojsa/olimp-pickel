...
export function ToolManagementDialog({
  ...
  const fetchCategories = async () => {
    try {
      ...
      const {
        data: dbCategories,
        error
      } = await supabase.from('tool_category_hierarchy').select('*').order('title');
      if (error) throw error;
      const {
        data: specFields,
        error: fieldsError
      } = await supabase.from('tool_spec_fields').select('*');
      ...
      dbCategories?.forEach(cat => {
        const category: ToolCategory = {
          ...
          children: [],
          specFields: specFields?.filter(field => field.category_id === cat.id).map(field => ({
            id: field.id,
            title: field.title,
  ...
  const addCategory = async (parentId?: string) => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.from('tool_category_hierarchy').insert([{
  ...
  const deleteCategory = async (categoryId: string) => {
    try {
      ...
      const {
        error
      } = await supabase.from('tool_category_hierarchy').delete().eq('id', categoryId);
      if (error) throw error;
      await fetchCategories();
      toast({
        title: "Success",
  ...
  const handleImageUpload = async (categoryId: string, event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const {
        error: uploadError
      } = await supabase.storage.from('tool-category-images').upload(fileName, file);
      ...
      } = supabase.storage.from('tool-category-images').getPublicUrl(fileName);

      // Update category with image URL
      const {
        error: updateError
      } = await supabase.from('tool_category_hierarchy').update({
  ...
  const addSpecField = async (categoryId: string) => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.from('tool_spec_fields').insert([{
  ...
  const deleteSpecField = async (fieldId: string) => {
    try {
      ...
      const {
        error
      } = await supabase.from('tool_spec_fields').delete().eq('id', fieldId);
      if (error) throw error;
      await fetchCategories();
      toast({
        title: "Success",
          ...
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              ...
              marginLeft: `${level * 20}px`
                ...
                {hasChildren && <Button variant="ghost" size="sm" onClick={() => toggleExpanded(category.id)} className="h-6 w-6 p-0">
              ...
              <div className="flex items-center gap-1">
                ...
                <Button variant="ghost" size="sm" onClick={() => deleteCategory(category.id)} className="h-8 w-8 p-0 text-destructive hover:text-destructive">
                  <Trash2 className="h-3 w-3" />
                </Button>
              </div>
            </div>

            {editingCategory === category.id && <div className="mt-3 flex gap-2" style={{
            marginLeft: `${(level + 1) * 20}px`
          }}>
                <Input placeholder="New subcategory title" value={newCategoryTitle} onChange={e => setNewCategoryTitle(e.target.value)} className="flex-1" />
                <Button size="sm" onClick={() => addCategory(category.id)}>
                  Add
                </Button>
            ...
            {!hasChildren && <div className="mt-3" style={{
            marginLeft: `${(level + 1) * 20}px`
          }}>
                <div className="flex items-center justify-between mb-2">
                  <Label className="text-sm font-medium">Specification Fields</Label>
                  <Button variant="outline" size="sm" onClick={() => setEditingSpecFields(category.id)} className="h-7">
                ...
                <div className="mb-3">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {['Length', 'Diameter', 'Radius', 'Thickness', 'Angle', 'Number of Inserts'].map(preset => <Button key={preset} variant="outline" size="sm" onClick={() => addPresetSpecField(category.id, preset)} className="h-6 text-xs px-2">
                        {preset}
                      </Button>)}
                  </div>
                </div>
                ...
                {/* Existing Fields */}
                {category.specFields && category.specFields.length > 0 && <div className="space-y-1 mb-2">
                    {category.specFields.map(field => <div key={field.id} className="flex items-center justify-between bg-muted/50 p-2 rounded">
                        <span className="text-sm">{field.title}</span>
                        <Button variant="ghost" size="sm" onClick={() => deleteSpecField(field.id)} className="h-6 w-6 p-0 text-destructive hover:text-destructive">
                          <Trash2 className="h-3 w-3" />
                        </Button>
                      </div>)}
                  </div>}
                ...
                {/* Add New Field Form */}
                {editingSpecFields === category.id && <div className="flex gap-2">
                    <Input placeholder="Field title (e.g., Length, Diameter)" value={newFieldTitle} onChange={e => setNewFieldTitle(e.target.value)} className="flex-1" />
                    <Button size="sm" onClick={() => addSpecField(category.id)}>
                      Add
                    </Button>
                    <Button size="sm" variant="outline" onClick={() => {
                setEditingSpecFields(null);
