...
-- Adds invoice_type column and proforma invoice number function
-- ============================================================

-- 1. Add invoice_type column to invoices table
ALTER TABLE public.invoices
ADD COLUMN IF NOT EXISTS invoice_type TEXT NOT NULL DEFAULT 'invoice';

-- 2. Add check constraint (only if not exists)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 3. Create function to generate proforma invoice numbers (YY2-XX format)
...
-- ============================================================
-- Sales Order Generate column: link OC, invoices, documents to SO
-- ============================================================

-- Add sales_order_id to order_confirmations
...
-- ============================================================
-- Purchase Orders (same structure as Sales Orders, no Customer PO#)
-- ============================================================
...
CREATE POLICY "Enable all operations for everyone" ON public.purchase_orders FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "Enable all operations for everyone" ON public.purchase_order_items FOR ALL USING (true) WITH CHECK (true);
...
DECLARE current_year TEXT; sequence_num TEXT; max_sequence INTEGER;
BEGIN
  current_year := RIGHT(EXTRACT(YEAR FROM CURRENT_DATE)::TEXT, 2);
  SELECT COALESCE(MAX(CAST(RIGHT(purchase_order_number, 2) AS INTEGER)), 0) INTO max_sequence
  FROM purchase_orders WHERE purchase_order_number LIKE 'PO-' || current_year || '-%';
  sequence_num := LPAD((max_sequence + 1)::TEXT, 2, '0');
